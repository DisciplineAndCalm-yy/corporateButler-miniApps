...
Page({
      ...
      complete: (res) => {
        ...
        if(res.confirm) {
          wx.requestSubscribeMessage({
            tmplIds: ['awvkbcpVTS0uFlglrbJh7RFo1cZCRQqPi3_RVE9uFsY'],
            success(res) {
              that.setData({
                showSubscribe: false
              });
  ...
  getMatchPolicyData() {
    ...
    let url = '/policy/calculator/matchingResultPageList';
  ...
  toMatchDetail(e) {
    let isMatch = true;
    if(this.data.tabIndex == 1) isMatch = false;
    wx.navigateTo({
      url: '/pages/subpages/pages/policyMatchDetail/policyMatchDetail?isMatch='+ isMatch +'&notStore=false&policyId=' + e.currentTarget.dataset.policyid,
    })
  },
